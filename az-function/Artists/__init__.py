...
async def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not artist_id or not artist_id.isdigit():
        return func.HttpResponse(
            ...
            body="{\"error\": \"artist_id must be an integer\"}"
        )
    ...
    # Safe diagnostics: don't log keys, only lengths and whether KV reference remained unresolved
    ...
    if mismatch:
        return func.HttpResponse(
            status_code=401,
            mimetype="application/json",
            body="{\"error\": \"unauthorized\", \"reason\": \"api_key_mismatch\"}"
        )
    ...
    # Detect unresolved Key Vault reference for DISC0GS_TOKEN to avoid passing bogus header upstream
    ...
    if token_unresolved:
        ...
        return func.HttpResponse(
            status_code=503,
            mimetype="application/json",
            body='{"error":"secrets_unresolved","which":"DISCOGS_TOKEN"}'
        )
    ...
    # Discogs token style: Discogs token=XXXX
    if discogs_token:
    ...
    try:
        ...
        while attempt < 2:
            ...
            try:
                ...
                if attempt >= 2:
                    raise
                backoff = 0.25 * attempt  # 0.25 then 0.5
                logger.warning("transient_retry", extra={"artist_id": artist_id, "attempt": attempt, "backoff_s": backoff, "error_type": type(rerr).__name__})
        ...
        # Fallback other status codes (e.g., 301/400/etc.)
        ...
        logger.warning("Timeout contacting Discogs artists", extra={"artist_id": artist_id})
        ...
        # Network, DNS, TLS, connection errors
        logger.exception("RequestError contacting Discogs artists", extra={"artist_id": artist_id})
        msg = f',"detail":"{str(e)}"' if debug_errors else ""
        return func.HttpResponse(status_code=502, mimetype="application/json", body=f"{{\"error\":\"bad_gateway\",\"trace_id\":\"{trace_id}\"{msg}}}")
    except Exception as e:
        logger.exception("Unexpected error contacting Discogs artists", extra={"artist_id": artist_id})
        msg = f',"detail":"{str(e)}"' if debug_errors else ""
